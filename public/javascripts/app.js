	...
	if(!date){
		...
		for (var i = event.resultIndex; i < event.results.length; ++i) {
			if (event.results[i].isFinal) {
				...
				client.sendText(final_transcript);
				console.log("time diff: "+ (date.getTime() - (new Date()).getTime()));
				date = null;
				recognition.stop();
				/*recognition.stop();
			...
			if(!recognitionTimeOut){
				recognitionTimeOut = true;
				recognitionTimeOutFunction();
			}
		}
		//TODO get the result final_transcript

	};
	var recognitionTimeOut = false;
	var recognitionTimeOutFunction = function(){
		$setTimeout(function(){
			recognition.stop();
			recognitionTimeOut = false;
			console.log("is recognition stop");
		},5000);
	}
